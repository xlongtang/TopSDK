...
{
    /// <summary>
    /// TOP API: taobao.sp.content.getpage
    /// </summary>
    public class SpContentGetpageRequest : ITopRequest<SpContentGetpageResponse>
    {
        /// <summary>
        /// JOSN格式的分页参数：  page： 查询起始页 >=1   rows： 分页大小 <= 50  sort : 排序字段，支持的排序类型包括：gmtModified – 最后修改时间；likeNum – 喜欢数；viewNum – 浏览数；  order：排序方式：asc – 顺序；desc – 倒序
        /// </summary>
        public string Pagerequest { get; set; }

        /// <summary>
        /// JSON格式的查询条件：  type(Integer，选填) ：内容类型，包括三种： 1 -- 宝贝，2 -- 图片，3 -- 自定义，4 -- 图文；     className(String，选填)：内容的自定义分类名称；     tags(String，选填)：内容的自定义标签名称，多个标签以逗号“,”分割。如果设置了多个标签，则他们是或的关系；     title(String，选填): 内容标题，此条件为模糊搜索， 慎用，可能比较慢；
        /// </summary>
